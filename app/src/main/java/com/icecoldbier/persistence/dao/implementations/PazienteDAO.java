...
public class PazienteDAO extends JDBCDAO<Paziente, Integer> implements PazienteDAOInterface {
    ...
    private static final String GET_ALL_RICETTE = "SELECT r.id, r.farmaco, r.prescritta FROM ricetta r, visita_base vb, report rep, visita_specialistica vs WHERE (r.id = vb.id_ricetta AND vb.id_paziente = ?) OR (r.id = rep.id_ricetta AND rep.id = vs.id_report AND vs.id_paziente = ?) LIMIT ? OFFSET ?;";
